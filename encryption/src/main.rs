...
fn encrypt(mut rng: OsRng, pk: &str, text: String) -> Vec<u8> {
    ...
    let public_key: RSAPublicKey = RSAPublicKey::from_pkcs1(&bytes).unwrap();
    println!("pasa sin romper public key");
...
fn decrypt(pk: &str, enc_data: Vec<u8>) -> Vec<u8> {
    
    let bytes = base64::decode(&pk).unwrap();
    
    let private_key = RSAPrivateKey::from_pkcs8(&bytes).expect("failed to parse key");
    
    let padding = PaddingScheme::new_oaep::<sha2::Sha256>();
    return private_key.decrypt(padding, &enc_data).expect("failed to decrypt");
}
